...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs map { vl => Try(vl) } onErrorReturn { throwable => Failure(throwable) }
     ...
     */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = obs concatMap {
      t => requestMethod(t) } recovered
