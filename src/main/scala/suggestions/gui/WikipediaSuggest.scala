...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.sanitized.concatRecovered { term =>
      wikiSuggestResponseStream(term)
    }

    // TO IMPLEMENT, update the suggestionList
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe (
      x => suggestionList.listData = x match {
        case Success(result) => result
        case Failure(ex) => List(ex.getMessage)
      },
      e => suggestionList.listData = List(e.getMessage),
      () => suggestionList.listData = List("Done")
    )

    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map {
      click => if (suggestionList.selection.items.size == 0) {
    ...
    val pages: Observable[Try[String]] = selections concatRecovered {
      selectionz => wikiPageResponseStream(selectionz)
    }

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(pag) => editorpane.text = pag
